...
from server import (
    assert_input_type,  # This will be the mocked instance
    assert_mode,
    find_place,
    get_directions,
    get_distance,
...
def test_assert_input_type_invalid():
    ...
    assert "'email' is not one of the allowed inpute types" in str(
        excinfo.value
    )  # Typo "inpute" is in original code
...
async def test_get_distance_success(mock_gmaps):
    ...
    mock_api_response = {
        "rows": [
            {
                "elements": [
                    {"distance": {"text": "50 km"}, "duration": {"text": "1 hour"}}
                ]
            }
        ]
    }
...
async def test_get_distance_no_results(mock_gmaps):
    ...
    default_fields_mode = (
        "driving"  # Default mode used by get_distance if not specified
    )
    ...
    mock_gmaps.distance_matrix.assert_called_once_with(
        origin, destination, default_fields_mode
    )
    assert (
        result_empty_rows
        == "No distance information found for the specified locations."
    )
...
async def test_get_geocode_success(mock_gmaps):
    ...
    mock_api_response = [{"geometry": {"location": {"lat": 37.7749, "lng": -122.4194}}}]
    ...
    expected_output = {"lat": 37.7749, "lng": -122.4194}
    assert result == expected_output
...
async def test_find_place_no_candidates(mock_gmaps):
    ...
    mock_gmaps.find_place.assert_called_once_with(
        query,
        "textquery",
        ["place_id", "formatted_address", "name", "geometry", "types", "rating"],
    )
...
async def test_place_nearby_no_results(mock_gmaps):
    ...
    assert (
        result["message"]
        == "Nothing nearby that matches the search criteria was found."
    )
    ...
    mock_gmaps.places_nearby.assert_called_once_with(location, radius, place_type)
    assert (
        result_for_none_api_response["message"]
        == "Nothing nearby that matches the search criteria was found."
    )
...
async def test_place_details_not_found(mock_gmaps):
    ...
    assert (
        result_empty_details["message"] == "No details found for the specified place."
    )
    ...
    assert (
        result_empty_details_no_name_request["message"]
        == "No details found for the specified place."
    )
    ...
    mock_gmaps.place.return_value = {
        "result": {"formatted_address": "Some Address"}
    }  # 'name' is missing
    ...
    assert (
        result_missing_name["message"]
        == "Essential place details (e.g. name) are missing."
    )

    # Scenario 3: API returns None (overall falsy response for 'results')
    mock_gmaps.reset_mock()
    mock_gmaps.place.return_value = None
    result_none_response = await place_details(
        place_id, fields
    )  # fields can be anything here
    mock_gmaps.place.assert_called_once_with(place_id, fields)
